...
import DeleteSelesai from './DeleteSelesai';
...
const SelesaiTable: React.FC = () => {
    const [currentPage, setCurrentPage] = useState(1);
    const [entriesPerPage, setEntriesPerPage] = useState(10);
    const [searchTerm, setSearchTerm] = useState('');
    const handleDelete = (id: number) => 
    
        console.log(`Deleting selesai with ID: ${id}`);
                    ...
                    <tbody>
                        {currentEntries.map((transaction, index) => (
                            <tr key={transaction.id} className="border-b">
                                ...
                                <td className="px-4 py-4">Rp. {transaction.denda.toLocaleString()}</td>
                                <td className="px-4 py-4">Rp. {transaction.totalDenda.toLocaleString()}</td>
                                <td className="p-2 text-center">
                                    <span className={`rounded px-3 py-1 text-sm ${getStatusColor(transaction.status)}`}>{transaction.status}</span>
                                </td>
                                <td className="p-2 text-center">
                                <DeleteSelesai selesai={SelesaiTable} onDelete={handleDelete} />
                                </td>
                            </tr>
                        ))}
                    </tbody>
